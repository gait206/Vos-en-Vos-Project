<?php
	...
	/*
		- NL-NL -
		PHP4 bibliotheek om een betaalverzoek te starten. 
		Berekend en controleerd de beveiligings hash.


		- EN-US -
		PHP4 Class to generate an OmniKassa form for a payment request. 
	...
	*/
	...
	class OmniKassa
	{
		...
		var $sSubId = '0';
		var $sSecurityKey = '';
		var $sSecurityKeyVersion = '1';

		var $sAquirerUrl = 'https://payment-webinit.omnikassa.rabobank.nl/paymentServlet';
		var $bTestMode = false;
		...
		var $sOrderId = '';
		var $sTransactionReference = false;
		...
		function OmniKassa($bTestMode = false)
		{
			if($bTestMode)
			{
				$this->setTestMode();
		...
		function setPaymentMethod($sPaymentMethods)
		{
			...
			if(strlen($sPaymentMethods) > 0)
			{
				...
				$sPaymentMethods = str_replace('CREDITCARD', 'VISA,MASTERCARD,MAESTRO', $sPaymentMethods); // Fix "creditcard"
		...
		function setAmount($fOrderAmount, $sCurrencyCode = 'EUR')
		{
		...
		function setSecurityKey($sSecurityKey, $sSecurityKeyVersion = '1')
		{
		...
		function setTransactionReference($sTransactionReference)
		{
		...
		function getCurrencyNumber($sCurrencyCode)
		{
		...
		function getTransactionStatus($sTransactionCode)
		{
